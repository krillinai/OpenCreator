...
```powershell
$env:CLAWEE_DESKTOP_TARGET_PLATFORM = "win"
$env:CLAWEE_DESKTOP_TARGET_ARCH = "x64"
...
├── Clawee Setup <version>.exe.blockmap
└── latest.yml
```
...
```powershell
Get-AuthenticodeSignature ".\Clawee Setup <version>.exe" | Format-List
```
...
4. `pnpm --filter @clawee/desktop e2e:package`。
5. `pnpm --filter @clawee/desktop verify:package`，由打包脚本自动调用。
6. 上传安装包、更新元数据和构建清单。
