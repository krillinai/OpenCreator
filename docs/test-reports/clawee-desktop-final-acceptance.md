...
| Desktop 类型检查 | PASS | `pnpm --filter @clawee/desktop typecheck` |
| 全离线 fresh package | PASS | `CLAWEE_DESKTOP_OFFLINE=1 CSC_IDENTITY_AUTO_DISCOVERY=false pnpm --filter @clawee/desktop package`；生产依赖下载量为 0 |
| 构建清单 | PASS | `apps/desktop/release/clawee-desktop-build-manifest.json` 记录 commit、dirty、时间、平台、架构和唯一包根目录 |
| 正式包独立校验 | PASS | `pnpm --filter @clawee/desktop verify:package` |
| Desktop packaged E2E | PASS | Playwright 6/6，使用构建清单指向的 fresh package |
| 真实 Codex hello Smoke | PASS | `CLAWEE_RUN_REAL_CODEX_SMOKE=1 pnpm --filter @clawee/desktop e2e:real-codex`；1/1 通过 |
...
2. 这些目录由 `.git/info/exclude` 排除，不进入仓库和 CI checkout，不属于 Clawee 发布源码。
...
| 场景 | 状态 | 证据 |
|---|---|---|
| 成功 Probe 后进入工作台 | PASS | Packaged E2E |
| Probe 失败停留诊断页 | PASS | `CODEX_PROBE_EXIT_NON_ZERO` 和重试计数断言 |
| 工作台握手失败恢复 | PASS | 本地错误页重载后不改变 Daemon PID，不重复 Probe |
...
| Renderer 不获得 Daemon Token | PASS | 连接配置仅返回 `/.clawee/runtime` |
...
- 包根目录：`apps/desktop/release/mac-arm64/Clawee.app`
...
5. 真实 Codex 报告位于 `test-results/clawee-desktop-real-codex-smoke.json`。
