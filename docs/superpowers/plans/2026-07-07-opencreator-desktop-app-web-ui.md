# OpenCreator Desktop App Web UI Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 把现有 `apps/web` 从旧四栏 Runtime Dashboard 改造成 OpenCreator 桌面 Agent 第一版 UI，保留已实现的 Runtime/SSE/service 能力，替换为截图方向的左侧栏、主对话、按需详情和极简设置。

**Architecture:** 前端继续使用 React + TypeScript + Vite。将现有 `AppLayout` 演进为桌面 App shell：固定左侧栏、中间会话区、右侧详情抽屉；Runtime、SSE、mock service 继续通过 service/adapter 接入，组件不直接调用 fetch 或本机能力。品牌、文案和设置收敛为 OpenCreator，Codex 只在关于页高级信息出现。

**Tech Stack:** React 18、TypeScript、Vite、Vitest、Testing Library、lucide-react、`@opencreator/protocol`。
...
- `apps/web/src/features/shell/OpenCreatorSidebar.tsx`
  - OpenCreator 左侧栏，包含 macOS 窗口点、全局入口、项目、对话、底部设置/更新。
...
- `apps/web/src/features/settings/OpenCreatorSettingsView.tsx`
  - 极简设置页：常规、插件、关于 OpenCreator。
- `apps/web/src/features/settings/OpenCreatorSettingsView.test.tsx`
  - 设置导航和高级信息测试。
...
- `apps/web/src/app/App.tsx`
  - 组合新的 OpenCreator shell、conversation、detail panel、settings view。
  - 删除主 UI 中 Codex Runtime Dashboard 文案。
  - 保留 RuntimeClient、SSE、mock file/change service。
- `apps/web/src/app/app-state.ts`
  - 扩展当前视图、项目、详情抽屉状态。
- `apps/web/src/app/App.test.tsx`
  - 更新为 OpenCreator 桌面 UI 断言，不再期待四栏 Dashboard 。
...
- `apps/web/src/components/timeline/Timeline.tsx`
  - 将 assistant title 从 Codex/Mock Agent 改为 OpenCreator，隐藏 raw 技术输出默认展示。
...
- `apps/web/src/features/settings/SettingsView.tsx`
  - 可删除或作为兼容 re-export 到 `OpenCreatorSettingsView`。
- `apps/web/src/features/settings/SettingsView.test.tsx`
  - 替换为新设置测试，或迁移到 `OpenCreatorSettingsView.test.tsx` 后删除。
- `apps/web/src/styles/app.css`
  - 重写桌面 App 布局和视觉样式。
- `apps/web/src/styles/tokens.css`
  - 调整颜色、尺寸、字体 token。
- `apps/web/index.html`
  - title 改为 OpenCreator。
...
describe('project-model', () => {
  it('creates OpenCreator default projects with content-design selected by default', () => {
...
```bash
pnpm --filter @opencreator/web test -- src/features/projects/project-model.test.ts
```
...
export type OpenCreatorProject = {
  id: string;
  name: string;
  cwd: string;
  pinned: boolean;
...
export function createDefaultProjects(): OpenCreatorProject[] {
...
export function findProjectById(projects: OpenCreatorProject[], projectId: string): OpenCreatorProject | undefined {
  return projects.find(project => project.id === projectId);
}

export function listRecentConversations(): OpenCreatorConversation[] {
  return [
    {
...
```bash
pnpm --filter @opencreator/web test -- src/features/projects/project-model.test.ts src/app/app-state.test.ts
```
...
git commit -m "feat(web): add OpenCreator project UI model"
```
...
- Modify: `apps/web/src/components/layout/AppLayout.test.tsx`
- Create: `apps/web/src/features/shell/OpenCreatorSidebar.tsx`
- Create: `apps/web/src/features/shell/OpenCreatorSidebar.test.tsx`
- Modify: `apps/web/src/styles/app.css`

- [ ] **Step 1: 写布局测试**

更新 `apps/web/src/components/layout/AppLayout.test.tsx`，断言只有左侧栏、主区域、可选详情抽屉，不再有常驻文件树第四栏：
...
describe('AppLayout', () => {
  it('renders OpenCreator desktop shell with optional detail panel', () => {
    render(
      <AppLayout
        sidebar={<div>侧栏</div>}
        main={<div>主对话</div>}
    ...
    expect(screen.getByRole('complementary', { name: 'OpenCreator 导航' })).toHaveTextContent('侧栏');
    expect(screen.getByRole('main', { name: 'OpenCreator 工作区' })).toHaveTextContent('主对话');
...
将 `apps/web/src/components/layout/AppLayout.tsx` 改为：
...
export function AppLayout(props: {
  ...
  return (
    <main className={`opencreator-shell ${props.detailOpen ? 'has-detail' : ''}`}>
      <aside className="opencreator-sidebar-pane" aria-label="OpenCreator 导航">
        {props.sidebar}
      </aside>
      <section className="opencreator-main-pane" aria-label="OpenCreator 工作区">
        {props.main}
      </section>
      {props.detailOpen ? (
...
创建 `apps/web/src/features/shell/OpenCreatorSidebar.test.tsx`：
...
创建 `apps/web/src/features/shell/OpenCreatorSidebar.tsx`：
...
import type { OpenCreatorConversation, OpenCreatorProject } from '../projects/project-model.js';

type OpenCreatorSidebarProps = {
...
export function OpenCreatorSidebar(props: OpenCreatorSidebarProps) {
  return (
    <div className="opencreator-sidebar">
...
```bash
pnpm --filter @opencreator/web test -- src/components/layout/AppLayout.test.tsx src/features/shell/OpenCreatorSidebar.test.tsx
```
...
```bash
git add apps/web/src/components/layout/AppLayout.tsx apps/web/src/components/layout/AppLayout.test.tsx apps/web/src/features/shell/OpenCreatorSidebar.tsx apps/web/src/features/shell/OpenCreatorSidebar.test.tsx apps/web/src/styles/app.css
git commit -m "feat(web): add OpenCreator desktop shell"
```
...
describe('ConversationEmptyState', () => {
  ...
  it('falls back to the generic OpenCreator prompt without a project', () => {
    render(<ConversationEmptyState />);
    expect(screen.getByRole('heading', { name: '今天要让 OpenCreator 处理什么？' })).toBeInTheDocument();
  });
});
```
...
describe('Composer', () => {
  it('renders OpenCreator composer context and submits trimmed prompt', async () => {
    const user = userEvent.setup();
    ...
    expect(screen.getByText('open-opencreator')).toBeInTheDocument();
...
export function Composer(props: {
  ...
  return (
    <form
      className="opencreator-composer"
      onSubmit={(event) => {
...
.opencreator-composer textarea::placeholder {
  color: #b7b2aa;
}
...
```bash
pnpm --filter @opencreator/web test -- src/features/conversation/ConversationHeader.test.tsx src/features/conversation/ConversationEmptyState.test.tsx src/features/runs/Composer.test.tsx
```
...
git commit -m "feat(web): add OpenCreator conversation surface"
```
...
```tsx
it('labels assistant messages as OpenCreator and avoids Codex branding', () => {
  render(
    <Timeline
      items={[
        {
  ...
  expect(screen.getByText('OpenCreator')).toBeInTheDocument();
...
```bash
pnpm --filter @opencreator/web test -- src/components/timeline
```
...
git commit -m "feat(web): align timeline with OpenCreator task flow"
```
...
```bash
pnpm --filter @opencreator/web test -- src/features/details/DetailPanel.test.tsx
```
...
git commit -m "feat(web): add OpenCreator detail panel"
```
...
**Files:**
- Create: `apps/web/src/features/settings/OpenCreatorSettingsView.tsx`
- Create: `apps/web/src/features/settings/OpenCreatorSettingsView.test.tsx`
...
创建 `apps/web/src/features/settings/OpenCreatorSettingsView.test.tsx`：
...
- [ ] **Step 2: 实现 OpenCreatorSettingsView**

创建 `apps/web/src/features/settings/OpenCreatorSettingsView.tsx`：
...
export function OpenCreatorSettingsView(props: { runtimeStatus: RuntimeStatus; onBack(): void }) {
  ...
  return (
    <section className="settings-section">
      ...
      <div className="settings-card">
        ...
        <SettingRow title="菜单栏显示" description="关闭主窗口后，仍在菜单栏中保留 OpenCreator" value="开启" />
      </div>
    </section>
  );
}
...
function AboutSettings(props: { runtimeStatus: RuntimeStatus }) {
  return (
    <section className="settings-section">
      <h1>关于 OpenCreator</h1>
      <div className="settings-card">
        <SettingRow title="OpenCreator 版本" description="当前应用版本" value="0.1.0" />
        <SettingRow title="Runtime 版本" description="本地服务版本" value={props.runtimeStatus.runtimeVersion ?? '未检测'} />
        <SettingRow title="数据目录" description="OpenCreator 本地数据位置" value="应用数据目录" />
        <SettingRow title="检查更新" description="检查 OpenCreator 是否有新版本" value="可用" />
      </div>
      <h2>高级信息</h2>
      <div className="settings-card">
        ...
        <SettingRow title="本地运行内核状态" description="OpenCreator 调用本机能力的状态" value={props.runtimeStatus.connected ? '正常' : '未连接'} />
...
旧 `SettingsView.test.tsx` 如果和新 props 冲突，改为只验证 re-export 可渲染，或删除旧测试并只保留 `OpenCreatorSettingsView.test.tsx`。
...
git commit -m "feat(web): add simplified OpenCreator settings"
```
...
2. 主 UI 不应显示 `Codex Runtime Dashboard`。
...
如果连接状态仅在关于页展示，则通过点击 `设置 账户` 和 `关于 OpenCreator` 后再断言 `codex-cli test`。
...
```bash
pnpm --filter @opencreator/web test -- src/app/App.test.tsx
```
...
git commit -m "feat(web): assemble OpenCreator desktop app UI"
```
...
```bash
rg -n "Codex Runtime Dashboard|Mock Agent|连接 Runtime|Runtime 地址|Runtime Token|API Key|适用于编程|工作模式|English|自动检测" apps/web/src apps/web/index.html
```
...
Expected: 只在 `OpenCreatorSettingsView` 关于页高级信息、测试 fixture、runtime type/变量名中出现。不得在主导航、空状态、composer、timeline title 中出现。
...
```bash
pnpm --filter @opencreator/web dev
```
...
4. 输入框显示 `随心输入`、`完全访问`、`5.5 超高`、`content-design`、`本地模式`、`open-opencreator`。
5. 点击设置进入极简设置页。
6. 主界面无 Codex 产品名。
...
git commit -m "test(web): verify OpenCreator desktop UI"
```
...
- OpenCreator 品牌：Task 6、Task 7、Task 8 覆盖。
...
`ProjectPermission`、`ActiveView`、`RightPanelMode`、`OpenCreatorProject`、`OpenCreatorConversation` 在 Task 1 定义，后续任务复用同名类型。`AppLayout` props 在 Task 2 修改，Task 7 按新签名组装。
