# OpenCreator Skills Hub 插件市场实施计划

> **面向执行 Agent：** 必须使用 `subagent-driven-development`（推荐）或 `executing-plans`，按任务逐项执行并使用复选框追踪。不得使用 Git worktree。

**目标：** 将 Skills Hub 的 55 条审核目录原生整合到 OpenCreator“插件”Tab，接入真实全局 Skill 安装、更新和新对话草稿插入能力。

**架构：** 新增 `@opencreator/skill-market` 共享包作为 web 与 daemon 的单一审核目录；daemon 只接受市场条目 ID，下载固定 GitHub commit 后复用现有本地 Skill 安装器；web 负责市场浏览、状态合并和交互，并通过 App 创建新对话后向 Composer 写入 `$skill-id ` 草稿。
...
- 市场目录随 OpenCreator 发版更新；运行时不得远程更新目录。
...
```bash
pnpm --filter @opencreator/skill-market test
```

预期：失败，提示找不到 `@opencreator/skill-market` 包或 `src/index.ts`。
...
```bash
cp Skills-Hub/02_projects/opencreator-skills-hub/data/skills.json \
  packages/skill-market/src/source-skills.json
cp Skills-Hub/02_projects/opencreator-skills-hub/data/categories.json \
  packages/skill-market/src/source-categories.json
cp Skills-Hub/02_projects/opencreator-skills-hub/lib/custom-skills.ts \
...
```bash
pnpm --filter @opencreator/skill-market test
pnpm --filter @opencreator/skill-market typecheck
```
...
```bash
pnpm --filter @opencreator/daemon test -- codex-skill-market-records storage protocol-shape
```
...
```bash
pnpm --filter @opencreator/daemon test -- codex-skills-validator codex-skills-installer codex-skill-market-downloader
```
...
```bash
pnpm add --filter @opencreator/daemon yaml@^2.8.1 tar@^7.4.3
pnpm add --filter @opencreator/daemon @opencreator/skill-market@workspace:*
```
...
```bash
pnpm --filter @opencreator/daemon test -- codex-skill-market-manager api
```
...
```text
opencreator.skill-market.saved.v1
```
...
```bash
pnpm --filter @opencreator/web test -- skill-market-service skill-market-model skill-market-storage
```
...
```bash
pnpm add --filter @opencreator/web @opencreator/skill-market@workspace:*
```
...
```bash
pnpm --filter @opencreator/web test -- SkillMarketView
```
...
cp Skills-Hub/02_projects/opencreator-skills-hub/public/examples/gpt-image-2-info-poster.png \
  apps/web/public/skill-market/examples/
cp Skills-Hub/02_projects/opencreator-skills-hub/public/examples/nano-banana-pro-product-visual.png \
  apps/web/public/skill-market/examples/
cp Skills-Hub/02_projects/opencreator-skills-hub/public/examples/seedance-2-video-ad.png \
  apps/web/public/skill-market/examples/
```
...
独立站点的 `OpenCreator Skills Hub` Header 不移入应用。

- [ ] **Step 6：实现市场专用 CSS**

要求：

1. 使用 OpenCreator 的 `--bg`、`--surface`、`--surface-2`、`--text`、`--muted`、`--accent` token。
...
```bash
pnpm --filter @opencreator/web test -- Composer App
```
...
```bash
pnpm --filter @opencreator/web test -- Composer App SkillMarketView
