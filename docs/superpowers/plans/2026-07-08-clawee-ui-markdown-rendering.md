# Clawee UI Markdown Rendering Implementation Plan
...
**Goal:** 让 Clawee 对话、思考过程、工具/诊断详情和只读文件预览支持安全 Markdown 渲染，达到 `docs/superpowers/specs/2026-07-08-clawee-ui-markdown-rendering-design.md` 的验收标准。
...
```bash
pnpm --filter @clawee/web test -- --run apps/web/src/components/markdown/MarkdownRenderer.test.tsx
```
...
```bash
pnpm --filter @clawee/web test -- --run apps/web/src/components/timeline/Timeline.test.tsx
```
...
```bash
pnpm --filter @clawee/web test -- --run \
...
```bash
pnpm --filter @clawee/web dev -- --port 64100
```
...
5. Active run with no intermediate process shows `等待 Clawee 返回过程...`
...
- [ ] Brand consistency: UI remains Clawee; no Codex product branding is introduced.
