...
- 已完成标题和规范化会话正文的 SQLite FTS5 索引，覆盖用户消息、Clawee 回复、处理过程、工具调用、受限工具结果和文件变更。
...
- 自动化验证：
  - `pnpm --filter @clawee/daemon test` -> PASS，45 个测试文件、499 个测试通过，13 个真实 Codex smoke 按配置跳过。
  - `pnpm --filter @clawee/web test` -> PASS，49 个测试文件、353 个测试通过。
