# OpenCreator Agent 完整优化实施计划

> **文档用途：** 本文档是 OpenCreator Agent 从当前状态演进到完整可用 Agent 产品的唯一实施主计划。后续 Agent 应按批次逐项执行、验证、提交和更新状态，不得跳过阶段门禁。
>
...
将 OpenCreator 从“已具备 Codex Runtime、会话、文件工作区、Skill 市场等主要模块，但运行状态、长会话性能和产品闭环仍不完整”的状态，分阶段建设为一个具备以下能力的完整 Agent：
...
- 协议变更必须先修改 `@opencreator/protocol`，再实现 daemon 和 Web。
...
| `PASS` | 实施和当前批次自动化门禁通过；已明确延后的真实页面验收进入最终统一验收清单 |
...
5. 自 2026-07-12 起，用户明确要求所有剩余批次连续实施，浏览器与真实页面验收统一放到最终阶段；只要代码、专项测试、受影响包类型检查、构建和可执行的非浏览器 smoke 通过，批次可标记 `PASS` 并继续，延后项必须进入最终验收矩阵。
...
5. 按批次要求完成可执行的真实 daemon 验证；浏览器与页面人工验收记录到最终统一验收清单，不阻塞后续批次。
6. 记录发现的设计偏差。若偏差改变 API、数据模型或验收标准，先更新本文档再继续。

### 6.3 完成后

1. 将验证命令和结果写入该批次的“执行结果”。
2. 满足代码和自动化门禁后将状态改为 `PASS`，勾选批次；延后的浏览器验收必须在执行结果和最终验收矩阵中保留。
3. 使用该批次推荐的提交信息独立提交。
...
每个涉及 Web 行为的批次最终至少验证：

- 桌面：1440x900。
- 移动：390x844。

剩余批次的浏览器验证统一在 P2-B8 最终验收执行；单批实现阶段继续保留组件测试、CSS 契约、类型检查和构建证据。
...
pnpm --filter @opencreator/daemon test -- test/integration/api.test.ts -t "scheduler"
pnpm --filter @opencreator/daemon typecheck
pnpm --filter @opencreator/daemon build
```
...
- 自动化验证：
  - `pnpm --filter @opencreator/daemon test` -> PASS，460 个测试通过，13 个真实 smoke 测试按环境开关跳过。
  - `pnpm --filter @opencreator/daemon typecheck` -> PASS。
  - `pnpm --filter @opencreator/daemon build` -> PASS。
- 真实生产入口验证：
...
```bash
pnpm --filter @opencreator/web test -- src/features/runs/run-registry.test.ts
pnpm --filter @opencreator/web test -- src/app/App.test.tsx -t "run registry"
pnpm --filter @opencreator/web typecheck
pnpm --filter @opencreator/web build
```
...
- 自动化验证：
  - `pnpm --filter @opencreator/web test -- src/features/runs/run-registry.test.ts` -> PASS，6 个测试通过。
  - `pnpm --filter @opencreator/web test -- src/app/App.test.tsx` -> PASS，42 个测试通过。
  - `pnpm --filter @opencreator/web test` -> PASS，43 个测试文件、319 个测试通过。
  - `pnpm --filter @opencreator/web typecheck` -> PASS。
  - `pnpm --filter @opencreator/web build` -> PASS；主包约 1024.60 kB，gzip 326.49 kB，保留既有大 chunk 警告。
- 真实服务验证：
...
```bash
pnpm --filter @opencreator/daemon test -- test/integration/api.test.ts -t "fromSeq"
pnpm --filter @opencreator/web test -- src/runtime/sse.test.ts
pnpm --filter @opencreator/web test -- src/features/runs/run-event-controller.test.ts
pnpm --filter @opencreator/web test -- src/app/App.test.tsx -t "restore"
pnpm --filter @opencreator/web typecheck
```
...
- 自动化验证：
  - `pnpm --filter @opencreator/daemon test -- test/integration/api.test.ts -t "fromSeq|event replay|replays events"` -> PASS，8 个相关测试通过。
  - `pnpm --filter @opencreator/web test -- src/features/runs/run-event-controller.test.ts` -> PASS，4 个测试通过。
  - `pnpm --filter @opencreator/web test -- src/features/runs/run-event-replay.test.ts src/app/App.test.tsx -t "restore|disconnect|replay|refresh"` -> PASS，12 个相关测试通过。
  - `pnpm test` -> PASS。
  - `pnpm typecheck` -> PASS。
...
```bash
pnpm --filter @opencreator/web test -- src/app/App.test.tsx -t "switch"
pnpm --filter @opencreator/web test -- src/app/App.test.tsx -t "cancel"
...
- 失败测试已先确认：
  ...
  - A 的迟到取消失败会显示在 B。
- 自动化验证：
  - `pnpm --filter @opencreator/web test` -> PASS，45 个测试文件、333 个测试通过。
  - `pnpm --filter @opencreator/web typecheck` -> PASS。
  - `pnpm --filter @opencreator/web build` -> PASS；仅保留既有主包体积警告。
- 真实服务验证：
...
- `apps/web/src/app/App.test.tsx`
- 新增 `docs/superpowers/test-reports/2026-07-12-opencreator-p0-run-correctness.md`
...
- 真实 Codex smoke：
  - `OPENCREATOR_RUN_REAL_CODEX_SMOKE=1 pnpm --filter @opencreator/daemon test -- test/smoke/real-codex-smoke.test.ts` -> PASS，13 项全部通过。
- 真实服务验证：
...
让长会话和大量数据保持流畅，建立可分页、可搜索的数据基础，并把已有 daemon 能力完整呈现在产品页面中。P1 结束时，OpenCreator 应具备稳定的日常 Agent Dashboard 体验。
...
```bash
pnpm --filter @opencreator/daemon test -- test/unit/codex-session-indexer.test.ts
pnpm --filter @opencreator/daemon test -- test/unit/codex-sessions-scanner.test.ts
...
- 自动化验证：
  - `pnpm --filter @opencreator/daemon test` -> PASS；41 个测试文件、472 项通过，13 项真实 smoke 按默认开关跳过。
  - `pnpm --filter @opencreator/daemon typecheck` -> PASS。
  - `pnpm --filter @opencreator/daemon build` -> PASS。
...
```bash
pnpm --filter @opencreator/daemon test -- test/integration/api.test.ts -t "history pagination"
pnpm --filter @opencreator/daemon test -- test/unit/protocol-shape.test.ts
pnpm --filter @opencreator/web test -- src/services
pnpm --filter @opencreator/daemon typecheck
pnpm --filter @opencreator/web typecheck
```
...
```bash
pnpm --filter @opencreator/web test -- src/features/conversation/use-thread-history.test.ts
pnpm --filter @opencreator/web test -- src/components/timeline/Timeline.test.tsx
pnpm --filter @opencreator/web test -- src/app/App.test.tsx -t "history"
pnpm --filter @opencreator/web typecheck
pnpm --filter @opencreator/web build
```
...
- 自动化验证：
  - `pnpm --filter @opencreator/web test -- src/features/conversation/use-thread-history.test.ts` -> PASS，2 项。
  - `pnpm --filter @opencreator/web test -- src/components/timeline/Timeline.test.tsx` -> PASS，23 项。
  - `pnpm --filter @opencreator/web test -- src/app/App.test.tsx -t "history"` -> PASS，5 项。
  - `pnpm --filter @opencreator/web test` -> PASS，47 个测试文件、343 项。
...
```bash
pnpm --filter @opencreator/daemon test -- test/unit/ordered-log-writer.test.ts
...
- 自动化验证：
  - `pnpm --filter @opencreator/daemon test` -> PASS，43 个测试文件、491 个测试通过，13 个真实 Codex smoke 按配置跳过。
  - `pnpm --filter @opencreator/daemon typecheck` -> PASS。
  - `pnpm --filter @opencreator/daemon build` -> PASS。
  - `git diff --check` -> PASS。
...
```bash
pnpm --filter @opencreator/daemon test -- -t "search"
pnpm --filter @opencreator/web test -- src/features/search/SearchView.test.tsx
pnpm --filter @opencreator/web typecheck
pnpm --filter @opencreator/daemon typecheck
pnpm build
```
...
- 已完成标题和规范化会话正文的 SQLite FTS5 索引，覆盖用户消息、OpenCreator 回复、处理过程、工具调用、受限工具结果和文件变更。
- 已排除二进制内容、超大工具输出、超长正文和 `done` 项，避免把不可安全展示或无搜索价值的原始内容写入全文索引。
- 搜索 API 已支持 cwd、内容类型、时间范围、不透明游标分页和安全高亮片段；分页请求不会重复触发会话目录同步。
- Web 搜索页已支持 250 ms 防抖、竞态隔离、加载/空结果/错误状态、分页、键盘导航和独立结果滚动。
- 点击搜索结果会按 `targetItemId` 加载目标附近的 50 条历史窗口，自动定位并高亮目标记录，不加载完整长会话。
- 旧数据库升级不再提升 Codex Session 主索引版本，也不重新解析 3.44 GB JSONL 或重写已有 item：
  - 首次缺失 FTS 时从 SQLite 规范化 item 一次性回填。
  - `codex_session_search_state` 持久化回填版本，后续重启仅执行 O(1) 状态读取。
  - 已有完整 FTS 的旧库只做一次完整性确认并写入状态标记。
- 会话目录同步增加 30 秒搜索新鲜度窗口，连续输入复用最近的线程列表同步；目标历史读取通过 inode、大小、mtime 和 parsed offset 判断单文件是否变化，未变化时直接读取 SQLite，变化时立即同步。
- 自动化验证：
  - `pnpm --filter @opencreator/daemon test` -> PASS，45 个测试文件、499 个测试通过，13 个真实 Codex smoke 按配置跳过。
  - `pnpm --filter @opencreator/web test` -> PASS，49 个测试文件、353 个测试通过。
  - daemon/Web 类型检查 -> PASS。
  - `pnpm build` -> PASS；仅保留既有 Web 大包 warning。
  - `git diff --check` -> PASS。
- 真实 3.44 GB 会话目录验证：
  - 2063 个会话、343703 条 FTS 记录、2063 个标题来源，搜索状态版本为 1。
  - 缺失 FTS 的一次性 SQLite 回填耗时 48.89 秒；不会阻塞首页和普通会话刷新。
  - 索引追平后的真实刷新约 1.03 秒，其中紧随线程列表的目标历史读取约 3 ms。
  - daemon 重启后的首次冷 FTS 查询约 1.1 秒；热查询 API 为 2 至 10 ms，Web 端到端约 299 ms（包含 250 ms 防抖）。
- 真实浏览器验证：
  - 中文短语、英文标识符和 `apps/web/src/app/App.tsx` 路径均能命中真实会话。
  - 桌面点击结果会携带 `targetItemId`，目标记录位于可视区且高亮。
  - 390×844 移动端结果区可滚动、无横向溢出，可从结果返回目标会话。
  - 验证期间浏览器控制台无错误。
- 提交：`8eb06ce`、`f30bec4`、`384d525`、`0183d67`。
- 已知边界：
  - 只有旧数据库完全缺失 FTS 时才会发生 48.89 秒的一次性回填；后续由持久化状态标记避免重复执行。
  - 首次冷 FTS 查询仍受 SQLite 页缓存影响，热查询已进入毫秒级。
...
```bash
pnpm --filter @opencreator/web test -- src/features/schedules
pnpm --filter @opencreator/daemon test -- test/unit/scheduler-service.test.ts
pnpm --filter @opencreator/web typecheck
pnpm --filter @opencreator/web build
```
...
- 自动化验证：
  - `pnpm --filter @opencreator/web test` -> PASS，50 个测试文件、359 个测试通过。
  - `pnpm --filter @opencreator/daemon test -- test/unit/scheduler-service.test.ts` -> PASS，19 个测试通过。
  - `pnpm --filter @opencreator/web typecheck` -> PASS。
  - `pnpm build` -> PASS；仅保留既有 Web 大包 warning。
  - `git diff --check` -> PASS。
- 真实浏览器验证：
...
- 对应测试文件
- `apps/web/src/features/settings/OpenCreatorSettingsView.tsx`
- `apps/web/src/features/runs/Composer.tsx`
- `apps/web/src/app/App.tsx`
- 必要时修改 `packages/protocol/src/api.ts`
...
```bash
pnpm --filter @opencreator/web test -- src/features/settings
...
```bash
pnpm --filter @opencreator/web test -- src/components/layout
pnpm --filter @opencreator/web test -- src/features/shell
pnpm --filter @opencreator/web test -- src/features/plugins
pnpm --filter @opencreator/web test -- src/styles/app-css.test.ts
pnpm --filter @opencreator/web build
```
...
**执行结果：** `PASS`。代码、自动化测试、类型检查、构建和真实服务健康检查已完成；1440x900 与 390x844 的真实交互和视觉验收按用户要求移入最终统一验收：
...
- 滚动与响应式：
  - `opencreator-main-content` 成为移动端统一滚动容器；会话 Timeline、搜索、Schedules、设置和 Skill 市场继续保留各自已有的内部滚动语义。
  - CSS 契约测试覆盖抽屉定位、打开状态、主内容滚动容器和移动 Skill 全高详情。
- 自动化验证：
  - `pnpm --filter @opencreator/web test -- src/app/App.test.tsx src/components/layout src/features/shell src/features/plugins src/styles/app-css.test.ts` -> PASS，7 个测试文件、134 项。
  - `pnpm --filter @opencreator/web test` -> PASS，57 个测试文件、388 项。
  - `pnpm --filter @opencreator/web typecheck` -> PASS。
  - `pnpm --filter @opencreator/web build` -> PASS；仅保留既有 Vite 主包超过 500 kB 警告。
...
- 自动化验证：
  - `pnpm --filter @opencreator/web test` -> PASS，57 个测试文件、395 项。
  - `pnpm --filter @opencreator/web typecheck` -> PASS。
  - `pnpm --filter @opencreator/web build` -> PASS。
  - `pnpm test` -> PASS。
  - `pnpm typecheck` -> PASS。
  - `pnpm build` -> PASS。
- 构建体积：
...
补齐多模态输入、任务 Steering、安全审批、通知恢复和显式记忆，使 OpenCreator 不仅能运行 Codex，还能完整管理用户任务、上下文和风险。
...
```bash
pnpm --filter @opencreator/daemon test -- test/unit/attachment-service.test.ts
pnpm --filter @opencreator/daemon test -- test/integration/api.test.ts -t "attachment"
pnpm --filter @opencreator/daemon typecheck
pnpm --filter @opencreator/web typecheck
```
...
- 自动化验证：
  - `pnpm --filter @opencreator/daemon test -- test/unit/attachment-service.test.ts test/unit/storage.test.ts` -> PASS，16 项。
  - `pnpm --filter @opencreator/daemon test -- test/integration/api.test.ts -t "attachment"` -> PASS。
  - `pnpm --filter @opencreator/daemon test` -> PASS，46 个测试文件、508 项；真实 Codex smoke 13 项按默认配置跳过。
  - `pnpm --filter @opencreator/web test` -> PASS，58 个测试文件、397 项。
...
```bash
pnpm --filter @opencreator/daemon test -- -t "images"
pnpm --filter @opencreator/web test -- src/features/runs/Composer.test.tsx
pnpm --filter @opencreator/web test -- src/features/runs/AttachmentTray.test.tsx
pnpm typecheck
pnpm build
```
...
- 自动化验证：
  - `pnpm --filter @opencreator/daemon test` -> PASS，46 个测试文件、511 项；真实 Codex smoke 13 项按默认配置跳过。
  - `pnpm --filter @opencreator/web test` -> PASS，59 个测试文件、405 项。
  - `pnpm typecheck` -> PASS。
...
```bash
pnpm --filter @opencreator/daemon test -- test/integration/run-manager.test.ts -t "queue"
pnpm --filter @opencreator/daemon test -- test/integration/run-manager.test.ts -t "interrupt"
pnpm --filter @opencreator/web test -- src/features/runs/Composer.test.tsx
pnpm --filter @opencreator/web test -- src/app/App.test.tsx -t "queued prompt"
pnpm typecheck
```
...
- 自动化门禁：
  - `pnpm --filter @opencreator/daemon test -- test/integration/run-manager.test.ts -t "interrupt|queue"` -> PASS，7 项。
  - `pnpm --filter @opencreator/daemon test -- test/integration/api.test.ts -t "follow-up modes"` -> PASS，2 项。
  - `pnpm --filter @opencreator/daemon test` -> PASS，46 个测试文件、515 项；真实 Codex smoke 13 项按默认配置跳过。
  - `pnpm --filter @opencreator/web test` -> PASS，59 个测试文件、407 项。
  - `pnpm typecheck` -> PASS。
...
```bash
pnpm --filter @opencreator/web test -- src/features/files/HtmlPreview.test.tsx
pnpm --filter @opencreator/web test -- src/features/files/FileEditorPane.test.tsx
pnpm --filter @opencreator/daemon test -- test/unit/workspace-files.test.ts
pnpm --filter @opencreator/web typecheck
pnpm build
```
...
- 自动化门禁：
  - `pnpm --filter @opencreator/web test -- src/features/files/HtmlPreview.test.tsx` -> PASS，6 项。
  - `pnpm --filter @opencreator/daemon test -- test/unit/workspace-files.test.ts` -> PASS，65 项。
...
```bash
pnpm --filter @opencreator/daemon test -- -t "approval"
pnpm --filter @opencreator/web test -- src/features/approvals
pnpm --filter @opencreator/web test -- src/app/App.test.tsx -t "approval"
pnpm test
pnpm typecheck
```
...
```bash
pnpm --filter @opencreator/daemon test -- -t "non-terminal"
pnpm --filter @opencreator/daemon test -- -t "restart recovery"
pnpm --filter @opencreator/web test -- src/features/tasks
pnpm --filter @opencreator/web test -- src/services/notification-service.test.ts
pnpm typecheck
pnpm build
```
...
- 新增全局任务 API：
  - `GET /tasks` 聚合 Run 和待审批状态，支持 `all`、`active`、`terminal`、`waiting_approval` 及各 Run 状态筛选。
  - 使用创建时间和 ID 组成稳定游标，单页上限 100，不需要 Web 扫描全部线程。
  - 返回线程标题、工作目录、Profile、排队位置、开始/结束时间、错误和终止原因。
- 新增正式任务中心：
  - 提供全部、进行中、待审批、已结束和失败筛选，以及稳定的“加载更多”分页。
  - 展示任务状态、线程、开始时间、耗时、工作目录和错误；待审批任务可直接批准或拒绝。
  - 点击任务会加载对应线程并打开 Run 详情，非首屏线程也可按需获取。
  - 新增 `#/tasks` 可复制路由、页面级懒加载、侧栏入口和未读数量徽标。
- 新增可选系统通知：
  - 浏览器权限只能由用户在任务中心主动开启，关闭后不再发送。
  - 第一次任务快照只建立静默基线；后续进入待审批、成功、失败或取消时才标记未读。
  - 当前前台会话不重复发送系统通知；其他页面或其他线程通过 Host Bridge 通知，点击通知进入对应线程。
  - 未读 ID 和通知设置保存在本地存储，可单条已读或全部清除。
- daemon 重启恢复：
  - 沿用并补充验证既有启动恢复逻辑；无法恢复的非终态 Run 明确收敛为 `failed/daemon_restart`，同时写入 done 事件。
  - 待审批在 daemon 重启或 Run 终止时同步收敛，不留下永久 pending。
- 自动化验证：
  - P2-B6 daemon 专项 -> PASS，任务 API、分页筛选和重启恢复共 7 项通过。
  - P2-B6 Web 专项 -> PASS，任务页、路由、侧栏、通知、未读和跳转共 41 项通过。
  - `pnpm test` -> PASS；daemon 532 项通过、13 项真实 Codex smoke 按配置跳过，Web 431 项通过，Skill Market 6 项通过。
  - `pnpm typecheck` -> PASS。
  - `pnpm build` -> PASS；任务中心独立分块为 6.73 kB / 2.74 kB gzip，仅保留既有主入口和 FilesPage 大分块警告。
  - `git diff --check` -> PASS。
- 延后到 P2-B8 最终统一验收：
  - 后台线程真实完成、失败、取消和待审批通知。
  - 点击真实系统通知和任务项跳转。
  - Run 执行中停止并重启 daemon 后的真实状态收敛。
  - 1440x900 与 390x844 的任务筛选、分页、审批、滚动和未读徽标布局。
...
```bash
pnpm --filter @opencreator/daemon test -- -t "memory"
pnpm --filter @opencreator/daemon test -- -t "summary"
pnpm --filter @opencreator/web test -- src/features/settings/MemorySettingsView.test.tsx
pnpm --filter @opencreator/web test -- src/features/conversation/MemorySuggestion.test.tsx
pnpm test
pnpm typecheck
```
...
```text
docs: finalize opencreator agent release readiness
```
...
- 最终报告：`docs/superpowers/test-reports/2026-07-12-opencreator-agent-final-acceptance.md`。
