...
4. Open Design 的品牌、文案、插件机制、文件打开协议都不能直接复用。
...
2. 用户消息支持 Markdown 渲染，但视觉上仍保持用户输入气泡的克制样式。
...
```ts
export type MarkdownVariant = "message" | "process" | "tool" | "diagnostic" | "document";
...
Timeline
  -> renderMessageContent
    -> MarkdownRenderer variant="message"
  -> renderProcessStep
...
5. 禁止：
   ...
   - `file:` 链接直接打开。
   - `vbscript:`
6. 外链默认 `target="_blank"`，并设置 `rel="noreferrer noopener"`。
7. Markdown 图片第一版不自动加载远程资源，避免模型输出触发隐私泄漏或不受控网络请求。
8. 复制代码块只复制纯文本 code body，不复制额外 DOM 文案。
...
1. `.md` 文件使用 `MarkdownRenderer variant="document"`。
2. `.json` 文件使用格式化代码块。
3. `.txt`、`.srt` 使用纯文本代码块或预格式文本。
4. `.html` 第一版不渲染为真实 HTML，显示源码或只读文本，避免和“禁止 HTML”策略冲突。
5. mock 文件预览仍需标记来源，不得暗示已经真实写盘。
...
```text
.markdown-prose
.markdown-prose[data-variant="message"]
...
6. 链接协议不允许时，渲染为普通文本或 disabled 链接，不触发导航。
...
6. run 运行中无过程消息时，显示“正在思考”和等待状态。
7. `diagnostic` JSON 展示为代码块，不当普通 prose。
8. 失败 `done` 默认展开并显示错误详情。
...
2. 最终回复、用户消息、思考过程都使用统一 Markdown 渲染。
...
8. 完成后思考过程默认折叠。
9. 文件预览中的 Markdown 文件使用同一 renderer。
10. 自动化测试覆盖 renderer、Timeline、文件预览和安全策略。
