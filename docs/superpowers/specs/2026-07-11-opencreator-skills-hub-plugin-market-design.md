# OpenCreator Skills Hub 插件市场设计

## 1. 状态

状态：设计已确认，等待用户审阅书面规格。

本文定义将 `Skills-Hub/02_projects/opencreator-skills-hub/` 的现有产品页面原生整合到 OpenCreator“插件”Tab 的第一版方案。

## 2. 背景

OpenCreator 当前“插件”Tab 仍为占位页面，但项目已经具备以下基础能力：
...
Skills Hub 当前的安装状态使用 `localStorage` 模拟，“去使用”通过 `opencreator://chat?skill=<skill-id>` 占位跳转。正式整合需要保留现有产品设计，并将占位状态和协议替换为 OpenCreator 的真实安装、版本和对话能力。

## 3. 目标

第一版目标：

1. 用 Skills Hub 市场替换 OpenCreator“插件”Tab 的占位页面。
2. 保留现有搜索、分类、场景、排序、收藏、视觉卡片和悬浮详情弹窗。
3. 市场目录随 OpenCreator 版本内置，并且只包含经过审核的条目。
...
7. 使用 OpenCreator 市场安装记录判断已知版本是否需要更新。
...
6. 嵌入 OpenCreator 后移除独立网站外壳，只保留市场内容区域。
...
1. 市场目录随 OpenCreator 发版更新，不在应用启动时远程拉取。
2. 每个可安装条目记录单调递增的市场修订号和已审核 Git commit。
3. 安装时下载清单指定的 commit，不安装远端分支最新内容。
4. 全局目录中的同名 Skill 即使由其他方式安装，也显示为已安装。
5. 有 OpenCreator 市场安装记录时，比较本地市场修订号与内置市场修订号。
6. 本地市场修订号较低时显示“更新”。
7. 外部安装且没有版本记录时显示“已安装，版本未知”，允许使用，不直接判定为低版本。

## 6. 方案选择

### 6.1 方案 A：原生整合到 OpenCreator

将 Skills Hub 的页面组件、样式和审核目录迁入 OpenCreator 前端，安装、更新和使用接入 OpenCreator 真实能力。

优点：

1. 与 OpenCreator 项目、对话和 Composer 状态直接集成。
...
1. 需要将独立页面适配到 OpenCreator 现有组件、主题和状态管理。

### 6.2 方案 B：iframe 嵌入

保留独立 Skills Hub 页面，通过 iframe 和消息协议调用 OpenCreator。
...
3. 不适合作为 OpenCreator 的核心本机能力。

### 6.3 方案 C：抽取共享前端包

将 Skills Hub 抽成独立组件包，由 OpenCreator 和原项目共同使用。
...
1. 首版工程量较大。
2. 当前只有 OpenCreator 一个明确的正式消费端。
3. 会提前引入发布、版本和跨项目兼容成本。

### 6.4 决策

采用方案 A：原生整合到 OpenCreator。
...
市场目录必须只有一个源码来源，由 OpenCreator web 和 daemon 构建时共同消费。
...
3. Skill 市场修订号：以 OpenCreator 市场安装记录为准。
4. 市场最新修订号：以内置市场目录为准。
5. 收藏：以 OpenCreator 本地持久化状态为准。
...
独立站点的品牌 Header 不进入 OpenCreator，避免与应用导航重复。
...
收藏是本机 OpenCreator 偏好，不影响真实 Skill 安装状态。
...
1. 调用 OpenCreator 现有新建对话能力，在当前选中的项目下创建新对话。
2. 等待新对话创建成功。
...
1. 下载和解压在 OpenCreator 管理的临时目录中完成。
...
2. 页面视觉和信息架构与现有 OpenCreator Skills Hub 保持一致。
