# OpenCreator UI Markdown 对话渲染设计 — 评审意见

评审对象：`docs/superpowers/specs/2026-07-08-opencreator-ui-markdown-rendering-design.md`

评审方式：逐条核对设计对现状的描述，并交叉阅读设计所引用的参考实现 `open-design-main/apps/web/src/runtime/markdown.tsx` 与 opencreator 当前 `Timeline.tsx`、`timeline-model.ts`、`app.css`、`package.json`。
...
- 建议：
  1. 设计第 8 章 `markdown-inline.tsx` 职责里明确列出"必须新增 `isSafeHref` 白名单校验",并注明**这是参考实现缺失、OpenCreator 必须补齐的部分**。
...
### 2.1 【落地】设计引用的 `FilePreview` 组件在 opencreator 中不存在

- 设计第 8 章架构图和第 13 章都以 `FilePreview` 为文件预览入口,第 19 章第 4 点写"修改文件预览 Markdown 分支"。
- 实际核对：opencreator web 里**没有**名为 `FilePreview` 的组件。文件内容当前由：
...
- 设计第 11.4/11.5 要求两种空态文案:运行中"等待 OpenCreator 返回过程..."、完成"本次没有可展示的中间过程。"
- 现状 `Timeline.tsx` 320 行:两种情况共用同一句 `等待 OpenCreator 返回结果...`(是"结果"不是"过程",且不区分状态)。
...
### 3.2 【依赖缺口】opencreator 无 clipboard 工具与 `Icon` 组件

- 参考 `MarkdownCodeBlock` 依赖 `copyToClipboard`(lib)、`useT`(i18n)、自研 `Icon`。核对 opencreator:无 `copyToClipboard` 工具、无 `useT`、图标用的是 `lucide-react`(已装,`^0.468.0`)。
- 设计第 14 章第 8 点说"复制按钮使用 lucide 图标"是对的(与 opencreator 技术栈一致),但第 8/10 章没提"需新增一个 clipboard 复制工具函数"这一实现依赖。建议在实施边界补一条,并注明复制失败态(第 16 章第 5 点)由谁承接。
...
9. opencreator 现有代码无 `dangerouslySetInnerHTML` —— 属实(grep 命中 0),说明第 9 条约束是"保持现状不破坏",方向正确。
