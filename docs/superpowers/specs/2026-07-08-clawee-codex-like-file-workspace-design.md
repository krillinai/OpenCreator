...
2. Open Design 的项目存储有 `.od/projects/<id>` 和外部项目目录两种模式，Clawee 第一版只围绕当前 thread `cwd`。
...
```text
clawee.files.recentOpenByWorkspace
clawee.files.expandedPathsByWorkspace
```
