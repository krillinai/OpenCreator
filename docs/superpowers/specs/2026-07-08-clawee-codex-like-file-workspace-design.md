# Clawee Codex-like 文件工作区设计

## 1. 背景

Clawee 当前已经具备 Agent runtime、项目、会话、Skills、MCP、定时任务和 Web UI 基础能力，但文件功能仍停留在前端 mock 层。
...
2. 文件树不是项目 `cwd` 的真实目录树。
3. 文件内容来自 mock IndexedDB，不来自当前项目目录。
4. 文件预览不是 Codex/Open Design 式文件工作区。
5. 没有真实保存、路径安全、权限校验、外部修改冲突处理。

本阶段目标是为 Clawee 设计第一版真实文件工作区。用户明确要求：
...
1. Open Design 的 `FileViewer` 绑定了 artifacts、导出、部署、评论、版本、品牌设计等大量业务，Clawee 第一版不能照搬。
2. Open Design 的项目存储有 `.od/projects/<id>` 和外部项目目录两种模式，Clawee 第一版只围绕当前项目 `cwd`。
...
Clawee 第一版应对齐这个形态，而不是继续使用当前 `DetailPanel` 作为文件预览主入口。

## 3. 目标

本阶段完成 Clawee 第一版真实文件工作区设计，为后续实现提供明确边界。

具体目标：

1. 文件功能访问当前项目 `cwd` 下的真实文件。
...
13. 外部编辑器深度集成。
14. 文件 watcher 实时同步。
15. 任意路径写入，即使项目权限是 `danger-full-access`，文件面板第一版也只允许访问当前项目 `cwd` 内路径。

## 5. 方案选择

### 5.1 方案 A：daemon 真实文件 API + Codex-like 文件工作区

daemon 增加真实文件系统 API，前端新增 `FileWorkspaceView`。文件根来自当前项目 `cwd`，所有路径限制在该根目录内。

优点：

1. 是真实本机文件能力，不是 mock。
2. 符合 Clawee 作为本机 Agent 桌面应用的方向。
...
web service
  ...
  ├── listTree(projectId)
  ├── getMeta(projectId, path)
  ├── openText(projectId, path)
  ├── saveText(projectId, path, content, baseMtimeMs)
  ├── blobUrl(projectId, path)
...
第一版 API 使用 `projectId`，前端不直接传绝对路径作为文件根。

后端解析规则：

1. 根据当前 web 项目模型中的 `projectId` 找到项目 `cwd`。
2. 若项目来自 runtime thread，则使用 thread 的 `canonicalCwd`。
3. 若是预置项目，则使用项目配置中的 `cwd`，展开 `~` 后 realpath。
4. 若目录不存在，返回 `WORKSPACE_NOT_FOUND`。

实现要求：

1. daemon 必须新增 `WorkspaceProjectResolver`，把 `projectId` 解析为 daemon 可信的 `cwd/canonicalCwd/permission`。
2. runtime thread 派生项目从 thread repository 读取 `canonicalCwd`。
3. 预置项目不能只存在于前端静态列表，必须迁移为 daemon 可读取的项目配置或项目 API。
4. 前端不能把绝对路径作为可信文件根传给文件 API。
5. 若前端传来的 `projectId` 无法在 daemon 解析，返回 `PROJECT_NOT_FOUND`。
...
这部分直接吸收 Open Design `resolveSafeReal` 的思路，但在 Clawee 中重新实现。
...
1. 目录树扫描最多返回 `5000` 个节点。
2. 文本文件预览/编辑最大 `2 MB`。
3. JSON 格式化预览最大 `1 MB`。
4. 图片预览最大 `20 MB`。
5. PDF 预览最大 `50 MB`。
6. 单目录读取遇到权限错误跳过该目录并返回 warnings。

超过限制时返回 meta，但 `previewable=false` 或 `editable=false`，并给出明确原因。

### 7.6 API 契约

#### `GET /workspace/files/tree?projectId=xxx`
...
#### `GET /workspace/files/meta?projectId=xxx&path=docs/a.md`
...
#### `GET /workspace/files/content?projectId=xxx&path=docs/a.md`
...
1. 如果当前文件 `mtimeMs !== baseMtimeMs`，且 `overwriteConflict !== true`，返回 `409 FILE_CONFLICT`。
2. 前端提示用户重新加载、覆盖保存或取消。
3. 覆盖保存仍必须重新做路径和权限校验。

#### `GET /workspace/files/blob?projectId=xxx&path=assets/a.png`
...
第一版在 macOS 可使用 `open -R <file>` 或 `open <dir>`。失败时返回 `REVEAL_UNAVAILABLE`。
...
错误响应沿用 Clawee Runtime 现有 `{ error: { code, message, details } }` 结构。
...
6. 搜索时保留路径上下文。
7. 空目录显示“当前目录为空”。
8. 项目无文件显示“当前项目暂无可显示文件”。
9. 目录扫描截断时显示“文件较多，仅显示前 5000 项”。
...
3. 调用 `GET /workspace/files/tree?projectId=...`。
4. 读取本地最近打开文件状态。
5. 若最近文件存在，打开该文件。
6. 若不存在，按顺序选择：
   - `README.md`
...
4. 调用 `PUT /workspace/files/content`，带 `baseMtimeMs`。
...
export function createWorkspaceFileService(client: RuntimeClient) {
  return {
    listTree(projectId: string): Promise<WorkspaceFileTreeResponse>;
    getMeta(projectId: string, path: string): Promise<WorkspaceFileMeta>;
    openText(projectId: string, path: string): Promise<WorkspaceFileContentResponse>;
    saveText(input: WorkspaceFileSaveRequest): Promise<WorkspaceFileSaveResponse>;
    blobUrl(projectId: string, path: string): string;
...
```text
clawee.files.recentOpenByProject
clawee.files.expandedPathsByProject
```
...
1. `GET /workspace/files/tree` 返回真实临时目录树。
2. `GET /workspace/files/content` 返回文本内容。
3. `PUT /workspace/files/content` 写入真实文件。
4. `GET /workspace/files/blob` 返回图片/PDF content type。
...
2. 点击 `.md` 显示内容并可切换预览。
3. 点击 `.ts` 显示源码和行号。
4. 编辑并保存调用 `PUT /workspace/files/content`。
5. 409 冲突显示处理入口。
6. 点击图片显示图片预览。
...
3. 接入项目切换、最近打开文件、展开状态。
4. 移除文件主路径对 mock `file-service` 的依赖。

### R3：Codex-like 文件工作区 UI

1. 实现 `FileWorkspaceView`。
2. 实现顶部栏、breadcrumb、文件树。
3. 实现文本编辑器、Markdown/JSON/源码预览。
4. 实现图片/PDF/不可预览状态。
...
对策：实现阶段先定义统一的 `WorkspaceProjectResolver`。文件 API 只接受 `projectId`，daemon 负责解析并校验 cwd。预置项目配置必须进入 daemon 或 runtime 数据库，不能让前端静态 cwd 成为信任来源。

### 17.2 文件树扫描太慢

风险：大型项目扫描卡顿。

对策：忽略常见大目录，设置节点上限，返回 truncated 和 warnings。后续再加懒加载目录。

### 17.3 编辑器复杂度过高

风险：一开始引入完整 Monaco/CodeMirror 可能拖慢进度。

对策：第一版可以先用轻量文本编辑器加行号和基础高亮；若现有依赖已具备成熟 editor，再复用。不能因为编辑器选型阻塞真实文件 API。
