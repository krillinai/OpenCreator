# OpenCreator UI Markdown 对话渲染设计

## 1. 背景

OpenCreator 当前前端对话区已经接入 Runtime、SSE、thread 历史和 `assistant_message` 思考过程展示，但消息正文仍然以纯文本 `<p>{text}</p>` 渲染。模型输出中的 Markdown 语法，例如 `**29°C**`、列表、代码块、表格和链接，会原样暴露在界面上，导致最终回复和思考过程都不够接近成熟 Agent 桌面应用的阅读体验。
...
本设计补充 `2026-07-07-opencreator-desktop-app-web-ui-design.md`，只处理 OpenCreator Web UI 的文本与 Markdown 渲染体系，不重做桌面壳、Runtime API、项目模型、会话模型或整体布局。
...
这些内容只能作为实现思路参考，不能照搬。OpenCreator 必须改写以下差异：

1. Open Design 显式 Markdown 链接没有做完整协议白名单校验；OpenCreator 必须补 `isSafeHref`。
2. Open Design 图片分支允许 `http/https/data:image/blob` 并产出 `<img>`；OpenCreator 第一版不能在聊天区主动加载远程图片。
3. Open Design 代码高亮使用 shiki 并通过 `dangerouslySetInnerHTML` 注入高亮 HTML；OpenCreator 第一版禁止采用这条路径。

### 2.2 不应直接搬迁的部分

1. `AssistantMessage.tsx` 绑定了 Open Design 的 artifacts、question form、analytics、brand browser、plugin、project file 等业务逻辑，不能直接进入 OpenCreator。
2. `ToolCard.tsx` 的工具族和 Open Design 业务耦合较多，只能参考“工具输出卡片化”的方法。
3. artifacts 里的 `renderMarkdownToSafeHtml` 是 HTML 链路，不符合 OpenCreator 本阶段禁止 HTML 的策略。
4. shiki 高亮 + `dangerouslySetInnerHTML` 链路不能复用。
5. Markdown 图片直接输出 `<img>` 的链路不能复用。
6. `ColorSwatch`、`PROSE_HEX_COLOR_RE` 色值色块和 `::code-comment{}` 指令块属于 Open Design 业务增强，不进入 OpenCreator 第一版。
7. Open Design 的品牌、文案、插件机制、文件打开协议都不能直接复用。

## 3. 目标

本阶段完成 OpenCreator UI 的统一 Markdown 渲染基础能力，让对话体验接近用户认可的 Open Design 效果，同时保持 OpenCreator 的产品边界和安全边界。

具体目标：

1. 最终 OpenCreator 回复支持 Markdown 渲染。
...
8. 伪造 Codex 或 OpenCreator 没有实际返回的思考内容。
9. 把 `reasoning_item` 作为思考过程唯一来源。OpenCreator 当前思考过程来源仍是 `assistant_message`。
...
1. `apps/web/src/components/timeline/Timeline.tsx`
   ...
   - 过程空态不区分运行中和完成，当前统一显示 `等待 OpenCreator 返回结果...`。
...
1. 参考 Open Design 的效果，但实现 OpenCreator 自己的轻量渲染层。
...
6. 样式统一走 OpenCreator 当前 tokens，不引入新的大型 UI 框架。
...
新增 OpenCreator 自己的 Markdown 渲染模块，参考 Open Design 的轻量 parser 和 React element 输出方式。它只支持 Agent 对话里高频 Markdown，不追求完整 CommonMark 兼容。
...
4. 样式和交互可控，代码块复制、长代码折叠、表格滚动都容易做成 OpenCreator 风格。
...
原因是 OpenCreator 当前需要的是 Open Design 那种聊天阅读体验，而不是完整 Markdown 文档系统。方案 A 能在不引入 HTML 字符串链路的前提下，快速补齐对话区最重要的阅读能力，并且可以和现有 Timeline 的 `assistant_message` 思考过程分流直接融合。
...
2. `markdown-inline.tsx`
   ...
   - 负责链接协议校验。
   - 必须实现 `isSafeHref` 白名单，这是 Open Design 参考实现缺失、OpenCreator 必须补齐的部分。
...
7. 运行中没有中间文本时，显示“思考中”，展开后显示“等待 OpenCreator 返回过程...”。
...
新增样式应集中在 `apps/web/src/styles/app.css` 或拆分后的 markdown 样式文件中，并使用现有 OpenCreator tokens。
...
1. 聊天回复字号和行高适合长文本阅读。
2. 思考过程更紧凑，字号略小，颜色更弱。
3. 代码块不使用大面积深色背景，保持 OpenCreator 浅色桌面应用质感。
4. 表格必须横向滚动，不能撑破对话列。
5. 链接要有可识别样式，但不能过于抢眼。
6. 行内代码不应导致行高跳动。
...
6. run 运行中无过程消息时，显示“正在思考”和 `等待 OpenCreator 返回过程...`。
