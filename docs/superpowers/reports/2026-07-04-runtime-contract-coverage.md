...
1. R0/R1 的基础 run 闭环通过：create run、SSE replay、cancel、history、diagnostics、fake Codex 异常路径和最小 real Codex smoke。
2. R2-R7 中的 thread/chat resume、profiles、skills、MCP pass-through、scheduler 和 release readiness 大部分还没有实现。
3. R-1 真实 Codex ABI 验证只完成了最小子集，尚未产出完整能力矩阵。
...
| Codex runner | `apps/daemon/src/codex/runner.ts` | `apps/daemon/test/integration/codex-runner.test.ts` | `PARTIAL` | 进程树强杀、spawnTimeout、inactivity 专项不足 |
...
| Run manager | `apps/daemon/src/runs/manager.ts` | `apps/daemon/test/integration/run-manager.test.ts` | `PARTIAL` | orphan 恢复、同 workspace/thread 串行、关键事件缺失判失败未完整实现 |
| Run API | `apps/daemon/src/api/routes.runs.ts` | `apps/daemon/test/integration/api.test.ts` | `PARTIAL` | `fromSeq`、terminal cancel 错误码、大量事件 replay、content-type 校验缺失 |
| SSE formatter | `apps/daemon/src/api/sse.ts` | `apps/daemon/test/integration/api.test.ts` | `PARTIAL` | SSE 心跳和 replay 行为缺专项测试 |
...
| Real Codex smoke | `apps/daemon/src/codex/smoke.ts` | `apps/daemon/test/smoke/real-codex-smoke.test.ts` | `PARTIAL` | 缺 resume/mcp/command/usage/failure/sandbox/image fixture |
...
| 事件协议稳定，SSE 可 replay | `PARTIAL` | 基础 replay 通过；真实 fixture 覆盖不足 |
...
| R-1 真实 Codex 验证通过，并保存 stdout/stderr 分离 fixture | `PARTIAL` | 只完成最小 smoke，未保存完整 fixture |
| 支持的 Codex 版本区间已明确，版本超界行为可验证 | `MISSING_IMPL` | 没有版本区间 gate |
| fake Codex 测试覆盖主要异常路径 | `PARTIAL` | 覆盖基础异常；缺 orphan、强杀、spawnTimeout、inactivity 专项 |
| 真实 Codex smoke 覆盖 assistant message、command execution、usage、stderr warning 和失败路径 | `PARTIAL` | assistant/stderr 间接覆盖；command/usage/failure 缺失 |
...
1. 先补 R-1 smoke 扩展和 fixture 保存：resume help、mcp help、command execution、失败路径。
2. 补 R1 contract 细节：`fromSeq`、`Last-Event-ID`、terminal cancel 错误码。
3. 补 R0 异常路径：orphan、强杀、inactivity、events 文件/DB 一致性。
