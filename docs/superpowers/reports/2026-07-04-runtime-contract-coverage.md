...
3. R-1 真实 Codex ABI 验证已覆盖 version/help/resume help/MCP help/最小 JSONL/command execution smoke，但尚未产出完整能力矩阵和落盘 fixture。
...
| Run manager | `apps/daemon/src/runs/manager.ts` | `apps/daemon/test/integration/run-manager.test.ts` | `PARTIAL` | 已覆盖 events 文件/DB seq 一致性和 inactivity；缺 orphan 恢复、同 workspace/thread 串行、关键事件缺失判失败 |
| Run API | `apps/daemon/src/api/routes.runs.ts` | `apps/daemon/test/integration/api.test.ts` | `PARTIAL` | 已覆盖 `fromSeq`、`Last-Event-ID`、terminal cancel 错误码；缺大量事件 replay、content-type 校验 |
| SSE formatter | `apps/daemon/src/api/sse.ts` | `apps/daemon/test/integration/api.test.ts` | `PARTIAL` | 已覆盖 full replay、`fromSeq`、`Last-Event-ID`、done 后关闭；缺心跳和运行中 tail 专项测试 |
...
| Real Codex smoke | `apps/daemon/src/codex/smoke.ts` | `apps/daemon/test/smoke/real-codex-smoke.test.ts` | `PARTIAL` | 已覆盖 resume help、mcp help、mcp add help、command execution；缺 usage/failure/sandbox/image 和落盘 fixture |
...
| 事件协议稳定，SSE 可 replay | `PARTIAL` | 基础 replay、`fromSeq`、`Last-Event-ID` 通过；真实 fixture 覆盖不足 |
...
| R-1 真实 Codex 验证通过，并保存 stdout/stderr 分离 fixture | `PARTIAL` | version/help/resume help/MCP help/JSONL/command execution smoke 通过；未保存完整 fixture |
...
1. 继续补 R-1 fixture 保存和失败路径：usage、failure、sandbox、stdout/stderr 落盘 fixture。
2. 补 R0 剩余异常路径：orphan 恢复、spawnTimeout 独立分类、进程树强杀专项。
3. 补 R1 剩余 contract 细节：大量事件 replay、运行中 tail、SSE 心跳、content-type 校验。
