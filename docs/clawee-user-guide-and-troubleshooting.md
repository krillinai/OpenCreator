...
1. 在“已安排”中手动创建任务，或选择“使用 Clawee 创建”并用自然语言描述任务。
...
6. 底层 Codex thread 失效或达到轮换阈值时可以更换，但侧栏任务入口和 Clawee Thread
   不会变化。
...
2. 请求 `http://127.0.0.1:9000/.clawee/runtime/healthz`，应返回 `{"ok":true}`。
...
出现 `401 token_expired` 或 `refresh_token_reused` 时，需要重新登录 Codex；不要通过删除 Clawee 数据规避登录问题。
...
该操作会移除 Clawee 本地线程、Run、附件元数据、审批、记忆和摘要。先完成备份，再停止服务并手工移动 `.runtime`。不要删除 `$CODEX_HOME`，除非明确要同时重置 Codex 本身。
