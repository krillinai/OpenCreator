# Clawee app-server 常驻化实施计划

> 状态：已完成（2026-07-28）
> 来源方案：`docs/specs/clawee-app-server常驻化方案-2026-07-28.md`
...
让用户主动发起的 Clawee 对话复用一个由 Daemon 管理的常驻 `codex app-server --stdio`，消除同 profile 热路径中的重复进程启动、`initialize` 和固定 MCP 初始化；后台定时任务继续使用现有一次性 app-server。
...
所有命令在仓库根目录 `/Users/wulien/develop/clawee/clawee-agent` 执行。

| 名称 | 命令 | 预期 |
|---|---|---|
| Capability 测试 | `pnpm --filter @clawee/daemon test -- test/unit/agent-capability-token.test.ts test/unit/agent-tool-run-injection.test.ts test/integration/agent-tool-api.test.ts` | 退出码 0 |
| Runner 测试 | `pnpm --filter @clawee/daemon test -- test/unit/codex-app-server-runner.test.ts test/unit/persistent-app-server-executor-2026-07-28.test.ts` | 退出码 0 |
| RunManager 回归 | `pnpm --filter @clawee/daemon test -- test/integration/run-manager.test.ts test/integration/approval-runtime.test.ts` | 退出码 0 |
| Daemon 全量测试 | `pnpm --filter @clawee/daemon test` | 退出码 0 |
| Daemon 类型检查 | `pnpm --filter @clawee/daemon typecheck` | 退出码 0 |
| 仓库类型检查 | `pnpm typecheck` | 退出码 0 |
| 真实 Codex smoke | `CLAWEE_RUN_REAL_CODEX_SMOKE=1 pnpm --filter @clawee/daemon test -- test/smoke/real-codex-smoke.test.ts` | 退出码 0 |
| Web 构建 | `pnpm --filter @clawee/web build` | 退出码 0 |
| Desktop 测试 | `pnpm --filter @clawee/desktop test` | 退出码 0 |
| Desktop 打包 | `pnpm --filter @clawee/desktop package` | 退出码 0，生成目录包 |
| Desktop 包校验 | `pnpm --filter @clawee/desktop verify:package` | 退出码 0 |
| Desktop E2E | `pnpm --filter @clawee/desktop e2e:package` | 退出码 0 |
| Desktop 真实 Codex | `CLAWEE_RUN_REAL_CODEX_SMOKE=1 pnpm --filter @clawee/desktop e2e:real-codex` | 退出码 0 |
...
- 修改：`apps/daemon/src/main.ts` - `CLAWEE_PERSISTENT_APP_SERVER`
...
2. 生产环境仅在 `CLAWEE_PERSISTENT_APP_SERVER=0` 时关闭常驻手动路径；不增加 UI 设置。
...
- RED：新增“uses one-shot manual execution only when CLAWEE_PERSISTENT_APP_SERVER=0”。
...
- `CLAWEE_PERSISTENT_APP_SERVER=0` 可恢复手动会话一次性执行；没有新增 Web/Desktop 平台分支、数据库迁移或旧 Codex 兼容层。
- 实现过程中自审并修复了五个边界问题：
...
| Desktop 目录包 | PASS：生成 `apps/desktop/release/mac-arm64/Clawee.app` |
