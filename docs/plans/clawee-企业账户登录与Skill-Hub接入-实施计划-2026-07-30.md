# Clawee 企业账户登录与 Skill Hub 接入实施计划

> 状态：实施完成（受控联调验收通过；正式发布被 HTTPS 门阻塞）
> 来源方案：`docs/specs/clawee-企业账户登录与Skill-Hub接入-方案-2026-07-30.md`
...
| BR-1 | 注册不传 `client_id`；注册成功后固定以 `client_id=clawee-agent` 自动登录。 |
...
| POST | `/enterprise/login` | 固定 `clawee-agent` 登录，验证权限后保存 Token。 |
...
生产实现使用 `@napi-rs/keyring@1.3.0` 的 `Entry.getPassword/setPassword/deletePassword`。默认 service 为 `com.clawee.enterprise`，account 为 `clawee-agent`。不提供任意 service/account 环境覆盖；实际打包 E2E 只允许传入规范 UUID `e2eRunId`，并固定派生 service=`com.clawee.enterprise.e2e`、account=`clawee-agent:<e2eRunId>`。安全后端异常统一包装为 `ENTERPRISE_SECURE_STORAGE_UNAVAILABLE`，禁止文件、SQLite、环境变量或普通配置降级。

实际打包 E2E 的授权契约固定为：

1. E2E 启动 App 时同时传入命令行参数 `--clawee-enterprise-e2e=<UUID>` 和环境变量 `CLAWEE_ENTERPRISE_E2E_RUN_ID=<同一 UUID>`，并把 `CLAWEE_ENTERPRISE_ORIGIN` 指向本机 loopback Fake 服务。
...
3. `DaemonManager` 必须从继承环境中删除 `CLAWEE_ENTERPRISE_E2E_RUN_ID`、`CLAWEE_ENTERPRISE_E2E_AUTHORIZED` 和任何 `CLAWEE_ENTERPRISE_KEYRING_SERVICE/ACCOUNT`，只根据已验证的 typed input 写入 `CLAWEE_ENTERPRISE_E2E_RUN_ID` 和 `CLAWEE_ENTERPRISE_E2E_AUTHORIZED=packaged-app`。
...
3. 本地有效但无 Clawee 来源记录：`installed_unknown_source`，动作 `use`。
...
- 新增 `#/account`，使用现有 WorkbenchLayout。
...
| 最近提交 | `456834d update clawee skill hub api docs` |
...
| 侧栏 | `apps/web/src/features/shell/ClaweeSidebar.tsx` | 拆分账户入口和设置按钮。 |
...
| Desktop E2E | `apps/desktop/e2e/packaged-app.ts`、`package-artifact.ts`、`desktop.spec.ts` | 复用实际包启动、重启、Runtime 代理和 `clawee-app://` 断言。 |
...
| 名称 | 运行目录与命令 | 预期 |
|---|---|---|
| V-PROTOCOL | 根目录：`pnpm --filter @clawee/protocol test && pnpm --filter @clawee/protocol typecheck && pnpm --filter @clawee/protocol build` | 退出 0 |
| V-DAEMON-UNIT | 根目录：`pnpm --filter @clawee/daemon test -- test/unit/<目标文件>` | 退出 0 |
| V-DAEMON-API | 根目录：`pnpm --filter @clawee/daemon test -- test/integration/enterprise-api-2026-07-30.test.ts` | 退出 0 |
| V-DAEMON | 根目录：`pnpm --filter @clawee/daemon test && pnpm --filter @clawee/daemon typecheck && pnpm --filter @clawee/daemon build` | 退出 0 |
| V-WEB-TARGET | 根目录：`pnpm --filter @clawee/web test -- <目标测试文件>` | 退出 0 |
| V-WEB | 根目录：`pnpm --filter @clawee/web test && pnpm --filter @clawee/web typecheck && pnpm --filter @clawee/web build` | 退出 0 |
...
| V-DESKTOP-E2E | 根目录：`pnpm --filter @clawee/desktop exec playwright test --config playwright.config.ts e2e/enterprise-packaged-2026-07-30.spec.ts` | 实际打包 App 企业流程退出 0 |
...
- 策略：必须。
- RED：`exports enterprise session and skill contracts without credential fields`。从 `@clawee/protocol` 公开入口导入并构造类型 fixture，先运行 V-PROTOCOL；预期因类型或错误码不存在而在 typecheck 阶段失败。此处是纯 TypeScript 契约被擦除后的受限例外，允许以 typecheck RED 证明缺失；会话、Hub 和账户运行时行为必须分别由 TASK-4、TASK-7、TASK-8 的业务断言取得真实 RED。
...
2. 注册 body 仅有 `email/name/password`；登录固定附加 `client_id: 'clawee-agent'`；注册响应 Cookie 不读取、不转发。
...
2. `resolveProductionServerEnvironment` 读取 `CLAWEE_ENTERPRISE_ORIGIN`，并按“实际打包 E2E 的授权契约”校验 `CLAWEE_ENTERPRISE_E2E_RUN_ID`、`CLAWEE_ENTERPRISE_E2E_AUTHORIZED`、UUID 和 loopback Origin；任何不完整或越权组合在构造 Store 前失败。
...
- 修改：`apps/web/src/features/shell/ClaweeSidebar.tsx`、`ClaweeSidebar.test.tsx`。
...
3. Browser 页面通过 `/.clawee/runtime-config` 获取同源 Runtime；Desktop 页面在应用脚本加载前注入最小 `window.claweeDesktop`，其 `readConnectionConfig` 指向同一 Runtime，其他原生能力只提供真实标识所需实现。
...
4. E2E 生成唯一 UUID，同时通过 `--clawee-enterprise-e2e=<UUID>`、`CLAWEE_ENTERPRISE_E2E_RUN_ID=<UUID>` 和 loopback `CLAWEE_ENTERPRISE_ORIGIN` 启动实际 App；Desktop Main 按授权契约生成 typed `enterpriseE2ERunId`。
5. DaemonManager 清除继承环境中的所有企业 E2E/Keyring 身份变量，只从 typed input 写入 Origin、run ID 和 `CLAWEE_ENTERPRISE_E2E_AUTHORIZED=packaged-app`；普通启动不得透传覆盖。
6. 打包 E2E 使用临时 CODEX_HOME 和由 UUID 派生的唯一 Keyring 项；通过 `clawee-app://app` 依次注册或登录、关闭并重启 App 恢复会话、打开企业列表、安装一个真实 ZIP Skill、使用或检查本地文件、注销清凭据。
...
- RED-4：实际 App spec 断言 `clawee-app://`、Runtime 登录、重启恢复、企业列表和安装；预期功能或打包资源缺失失败。
...
- Keyring `.node`、Web 哈希、Runtime 代理、`clawee-app://`、E2E 启动双信号、派生安全存储恢复和企业操作全部由当前工作区实际包证明；越权 E2E 配置在 Daemon 前失败，HTTP 正式发布失败、HTTPS 配置可继续；AC-5、AC-12、AC-13 通过。
...
| AC-2 | P0 | 注册并登录 | 未注册邮箱；Fake 服务记录请求 | 注册有效名称/邮箱/密码 | 注册无 client_id、Cookie 未使用；随后 `clawee-agent` 登录和 `/me` 验证；成功显示账户 | Runtime API + Web UI + 打包 App | 请求字段、状态流、截图 |
...
| AC-13 | P0 | 实际打包 App | 当前工作区重建包；唯一 UUID、匹配启动参数/环境和 loopback Fake Origin | 访问 `clawee-app://`，登录、重启恢复、列表、企业操作并校验包；另以不完整/越权 E2E 配置启动 | Preload/Runtime 代理可用；Keyring 从派生测试项恢复且生产项不受影响；越权配置在 Daemon 前失败；Web 文件列表/哈希一致 | V-PACKAGE + V-DESKTOP-E2E | 构建 manifest、verify JSON、派生身份、E2E trace |
...
- `pnpm --filter @clawee/desktop verify:package`：退出 0；包大小 531337447 字节，Daemon 43820011 字节，`@napi-rs/keyring-darwin-arm64/keyring.darwin-arm64.node`、SQLite、Electron fuses 和隐私约束通过。
- Desktop 企业契约测试：3 文件、13 测试通过；覆盖不完整/不匹配/非法/非 loopback E2E 配置拒绝、大小写无关环境清洗、Keyring 产物和 HTTP/HTTPS 门。
- V-DESKTOP-E2E：完整套件曾 12/12 通过；账户页简化后的新鲜包再次运行企业实际包规格 1/1 通过，耗时 27.6 秒，完成 `clawee-app://` 登录、重启恢复、真实 ZIP Skill 安装和使用、注销及再次重启保持未登录。
...
- 证据：[实施计划](/Users/wulien/develop/clawee/clawee-agent/docs/plans/clawee-企业账户登录与Skill-Hub接入-实施计划-2026-07-30.md:401) 规定先 `upsert` 企业记录、再清理临时目录，任一步失败都要恢复旧记录；但 TASK-5 的 Repository 仅定义 `getBySkillId/getByName/listRecords/upsertRecord`。现有 [`rollbackSkillInstall`](/Users/wulien/develop/clawee/clawee-agent/apps/daemon/src/codex/skills/manager.ts:158) 只恢复文件系统。
...
- 证据：[实施计划](/Users/wulien/develop/clawee/clawee-agent/docs/plans/clawee-企业账户登录与Skill-Hub接入-实施计划-2026-07-30.md:261) 规定 service/account 环境覆盖只允许测试环境；TASK-13 又要求实际生产形态的打包 App 通过环境变量使用唯一 E2E Keyring 项，但未定义环境变量名、测试身份信号及生产环境拒绝规则。当前 [`resolveProductionServerEnvironment`](/Users/wulien/develop/clawee/clawee-agent/apps/daemon/src/startup.ts:7) 也没有可复用的测试模式判定。
...
- 证据：[实施计划](/Users/wulien/develop/clawee/clawee-agent/docs/plans/clawee-企业账户登录与Skill-Hub接入-实施计划-2026-07-30.md:1003) 要求 TASK-12 的一致性测试因“功能尚未实现”取得 RED，但 TASK-12 排在账户、路由、Hub 和写操作全部实现完成之后。
