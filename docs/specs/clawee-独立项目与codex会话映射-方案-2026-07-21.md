...
当前 Clawee 的项目保存在 Web `localStorage`，普通会话从 Codex
app-server 的 `thread/list` 导入后，再按 `cwd` 与项目路径是否相等进行前端分组。
这同时混淆了三个不同概念：

1. Clawee 主动创建并管理的项目。
2. Clawee 主动创建的 RuntimeThread。
...
1. Clawee 项目与 Codex 项目彻底分离，双方不建立同步关系。
2. Clawee 只展示和管理自己创建或用户明确认领的普通会话。
3. Clawee 持久化项目与 RuntimeThread 的所有权关系，以及 RuntimeThread 与
   `codexThreadId` 的绑定。
4. Codex app-server 继续负责会话执行、恢复、正文历史和搜索来源。
5. 删除 `cwd` 运行期推断和“本机目录”默认项目，消除项目丢失与历史会话污染。
6. 升级时保留既有 Clawee 项目和自建会话，并提供待归属会话恢复入口。
...
1. “那就直接来个彻底的，clawee的项目和codex的项目彻底分离，二者不需要有关系，
   clawee维护自己创建的项目的会话，问题是会话是从codex cli读取的，由clawee维护
...
| 证据 | 已确认事实 |
|---|---|
| `apps/web/src/features/projects/project-model.ts` | 项目由 Web 模型定义并持久化到 `clawee.projects.v1` |
...
| `apps/daemon/src/codex/app-server-runner.ts` | `thread/start` 和 `thread/resume` 都接收 Clawee 提供的 `cwd` |

假设：

1. 一个普通 RuntimeThread 在任意时刻最多属于一个 Clawee 项目。
...
| 设计部分 | 核心决定 | 用户确认原话 |
|---|---|---|
| D-1 目标与产品边界 | 项目完全独立；移除“本机目录”；只展示 Clawee 拥有的会话；未知 Codex 会话不自动导入 | “没问题，继续” |
...
| ID | 类型 | 优先级 | 描述 |
|---|---|---|---|
| FR-1 | 功能 | P0 | 用户可以在 Clawee 中创建、查看、修改、归档和恢复独立项目 |
| FR-2 | 功能 | P0 | 新建普通会话必须指定活跃 `projectId`，Daemon 持久化项目归属 |
| FR-3 | 功能 | P0 | RuntimeThread 首次运行后绑定唯一 `codexThreadId`，后续从 Codex 读取和恢复历史 |
| FR-4 | 功能 | P0 | 侧边栏、搜索和导航只展示 Clawee 项目会话及现有调度会话 |
| FR-5 | 功能 | P0 | 升级后可查看并认领无法自动归属的 Clawee 自建会话 |
| FR-6 | 功能 | P1 | 用户可以显式更换项目目录，并让该项目既有普通会话使用新目录 |
| BR-1 | 规则 | P0 | Clawee 不读取、不修改、不同步 Codex 项目配置 |
...
| DEC-4 | Clawee 保存索引和所有权，Codex 保存会话正文 | 离线可见且不复制完整会话真相 | 列表、历史、恢复 |
| DEC-5 | 路径匹配只允许出现在一次性迁移中 | 保留旧数据但杜绝运行期启发式 | 升级迁移 |
| DEC-6 | 显式更换项目目录时更新该项目普通 Thread 的执行目录 | Codex resume 支持由 Clawee提供新 `cwd` | 项目目录修复 |
...
新项目使用 Clawee 生成的稳定 ID。新建时展开 `~`、校验目录并执行 `realpath`。
...
origin TEXT NOT NULL  -- clawee_created | codex_discovered
```
...
2. 创建带 `projectId`、`origin=clawee_created` 的 RuntimeThread。
...
普通 `/threads` 列表只查询本地 `origin=clawee_created` 且已有项目归属的记录，不再
...
打开搜索结果或深链接只能访问已存在的 Clawee RuntimeThread。
...
`clawee.projects.v1`，Daemon 在事务中创建项目、合并规范路径重复项，并仅将
`clawee_created` 历史普通会话按完整规范路径归属。旧 `local-home` 不创建项目；
...
| AC-7 | NFR-1, BR-4 | 已有项目和绑定会话 | 停止 Codex 后启动 Clawee | 索引仍显示，历史不可用但映射不丢失 | Desktop E2E |
