...
| 创建日期 | 2026-07-16 |
| 最近实施验收 | 2026-07-16 |
| 上游规格 | `docs/specs/2026-07-15-clawee-desktop-app-host-design.md` |
| 审查报告 | `docs/test-reports/clawee-desktop-final-acceptance.md` |
...
本文后续任务级验收清单保留原始验收定义。最终状态和本轮命令证据以本节及 `docs/test-reports/clawee-desktop-final-acceptance.md` 为准；未实际运行的平台项不得因代码已实现而视为 `PASS`。

## 1. 实施前复审结论

1. 原方案总体方向正确，但低估了开发代理、工作台空白页、通知重试、迁移 ABI、更新失败恢复和打包产物串用的复杂度。
...
4. 工作台是否可用不能只依赖 `loadURL()` 成功，必须增加 Renderer 就绪握手，否则脚本缺失或首屏异常仍可能表现为空白页。
...
5. 保证工作台加载失败后有本地恢复界面，并且恢复不重复 Probe。
...
1. Clawee 依赖用户本机 Codex CLI，不内置 Codex。
2. Clawee 不提供 Codex 登录页，不调用 `codex login status` 作为门禁。
3. 冷启动只通过一次真实最小 Codex 请求判断基础模型调用是否可用。
4. Probe 成功前不加载主工作台。
5. 页面刷新、项目切换、托盘恢复和 Daemon 自动恢复不重复 Probe。
6. Probe 不加载 Clawee Skills、Skill Market 或用户 MCP。
...
| P1-B5 故障恢复与诊断 | PARTIAL | 工作台缺少 Renderer 就绪握手、本地错误页和最终脱敏 |
...
1. Runtime 路由只匹配：
   - `/.clawee/runtime`
   - `/.clawee/runtime/...`
2. `/.clawee/runtimeevil` 等前缀碰撞不得进入代理。
...
1. `/.clawee/runtime-config` 只返回 `{ baseUrl: '/.clawee/runtime' }`。
...
### 4.11 工作台使用 Renderer Ready 握手

1. `loadURL()` 成功只表示主文档完成，不表示 React 工作台可用。
2. Web Renderer 首次挂载后通过 Preload 发送 `workspaceReady`。
3. Main 在加载工作台后等待固定握手超时；超时、主 frame 加载失败或 Renderer 崩溃都进入 `workspace_failed`。
4. 错误页只加载本地 Bootstrap 资源，提供：
   - 重新加载工作台。
   - 重启 Runtime。
   - 导出诊断。
   - 退出。
5. 重新加载工作台不重启 Daemon，不重复 Probe。
6. 重启 Runtime 复用已验证 Codex 指纹，不重复 Probe。
7. 错误页不得加载远程资源。

### 4.12 更新、打包和发布安全模型

1. 正式更新源固定为 GitHub Releases `wulien/clawee-agent`，由 Electron Builder 生成 packaged update 配置。
2. 生产运行时不依赖 `CLAWEE_UPDATE_URL`；测试使用注入的 fake updater，必要的开发覆盖不得进入正式包。
3. Updater 使用显式状态机：
...
```bash
pnpm --filter @clawee/desktop test -- runtime-proxy
pnpm --filter @clawee/desktop e2e:package --grep "Runtime 代理"
...
```bash
pnpm --filter @clawee/web test -- dev-proxy-target browser-bridge
...
CLAWEE_RUN_REAL_CODEX_SMOKE=1 pnpm --filter @clawee/desktop e2e:real-codex
```
...
```bash
pnpm --filter @clawee/daemon test -- codex-probe codex-process
pnpm --filter @clawee/desktop test -- daemon-manager
pnpm --filter @clawee/desktop e2e:package --grep "Probe"
```
...
pnpm --filter @clawee/desktop e2e:package --grep "启动性能"
```
...
```bash
pnpm --filter @clawee/desktop test -- data-migration
pnpm --filter @clawee/desktop e2e:package --grep "数据迁移"
pnpm --filter @clawee/desktop package
```
...
```bash
pnpm --filter @clawee/daemon test -- codex-capabilities startup
...
```bash
pnpm --filter @clawee/desktop test -- protocol-path static-response
pnpm --filter @clawee/desktop e2e:package --grep "静态资源"
```
...
```bash
pnpm --filter @clawee/desktop test -- workspace-ready
pnpm --filter @clawee/web test -- desktop-bridge
pnpm --filter @clawee/desktop typecheck
```
...
#### Task B7：增加工作台本地错误页和无 Probe 恢复

**实现内容：**

1. 在本地 Bootstrap 中增加工作台失败视图。
...
- [ ] 工作台资源缺失或握手超时时显示本地错误页。
- [ ] “重新加载工作台”不改变 Daemon PID 和 Probe 计数。
...
```bash
pnpm --filter @clawee/desktop test -- workspace-load
pnpm --filter @clawee/desktop e2e:package --grep "工作台加载失败"
```
...
```bash
pnpm --filter @clawee/desktop test -- notification-manager
pnpm --filter @clawee/daemon test -- notification
```
...
```bash
pnpm --filter @clawee/desktop test -- window-manager settings-store
```
...
```bash
pnpm --filter @clawee/desktop test -- logger redaction
```
...
```bash
pnpm --filter @clawee/desktop test -- diagnostics
```
...
```bash
pnpm --filter @clawee/daemon test -- bounded-buffer codex-runner codex-app-server
```
...
```bash
pnpm --filter @clawee/daemon test -- cors
pnpm --filter @clawee/desktop test -- deep-link
```
...
```bash
pnpm --filter @clawee/desktop test -- updater
pnpm --filter @clawee/desktop typecheck
```
...
- [x] `ELECTRON_RUN_AS_NODE=1 Clawee -e ...` 不再执行任意 Node 代码。
...
pnpm --filter @clawee/desktop e2e:package
```
...
pnpm --filter @clawee/desktop package
pnpm --filter @clawee/desktop verify:package
CLAWEE_DESKTOP_PACKAGE_ROOT="<当前构建清单中的包根目录>" pnpm --filter @clawee/desktop e2e:package
...
$env:CLAWEE_DESKTOP_PACKAGE_ROOT = "<win-unpacked>"
pnpm --filter @clawee/desktop e2e:package
```
...
- `docs/specs/2026-07-15-clawee-desktop-app-host-design.md`
...
6. 正式更新源不再使用 `wulien/clawee-agent` GitHub Releases。
...
- [x] 用户确认正式自动更新源使用 GitHub Releases `wulien/clawee-agent`。
