...
[主要功能](#主要功能) · [创作工具](#创作工具) · [产品展示](#产品展示) · [案例展示](#案例展示) · [快速开始](#快速开始) · [Desktop](#desktop) · [系统架构](#系统架构) · [开发指南](#开发指南) · [文档](#文档) · [Star 趋势](#star-趋势)
...
- 📥 **灵活获取视频**：支持上传本地素材，也可以通过专用导入和下载流程处理 YouTube、Bilibili 等平台的公开视频链接。
- 📜 **高精度语音识别**：可选择云端转写或本地 Whisper 方案，包括 OpenAI Whisper、FasterWhisper、WhisperKit、whisper.cpp 和阿里云语音。
- 🧠 **语义化字幕处理**：利用 LLM 上下文完成自然断句、对齐、校正和可编辑字幕，不只按固定字数机械切分。
- 🔄 **术语与字幕修正**：通过 Agent 指令统一人名和专业词汇，并在导出前逐条审核、修改字幕。
- 🌍 **上下文感知翻译**：结合前后语境完成翻译，支持目标语言和双语字幕设置，在完整视频中保持语义自然一致。
- 🎙️ **智能配音与声音克隆**：选择音色与表达风格、调整演绎方式，或提供自定义声音样本；可配置 OpenAI、MiniMax、阿里云和 Edge TTS 服务。
- 🎬 **自适应视频合成**：处理横屏与竖屏布局，自定义字幕位置、字体、字号和颜色，并分别导出字幕、配音或合成视频。
- ✂️ **智能剪辑**：分析长视频语义，从开头吸引力、完整度、情绪和传播潜力四个维度评估高光片段，再选择并导出可独立发布的内容。
- 🎨 **AI 图像与视频生成**：使用 GPT Image、即梦、可灵、Gemini、Seedance 或 Veo，并通过提示词、画幅、质量、分辨率、时长和生成数量产出可下载的视觉素材。
- 🎭 **数字人与故事动画**：制作数字人口播，或将角色与故事发展为可审核的分镜、配音、音乐和多版本动画成片。
- 🖼️ **封面与产出管理**：结合主题和参考素材生成多版封面，并在产出中心统一查看视频、图像、音频、字幕和文档。

### Agent 工作台

- 🔗 **工作台与对话同步**：通过共享工作流状态机，让表单操作、Agent 指令、执行进度和产出结果在两个界面中始终一致。
- 🕘 **版本化持续创作**：每次修正或重新生成都会创建新版本，不覆盖当前结果，并保留历史配置与产出供回看和比较。
- 🤖 **Codex 原生执行**：直接复用 Codex 的 Agent loop、模型、推理、工具调用、会话历史、技能、MCP 和 Profile，不维护第二套执行引擎。
- ⚙️ **后台持续运行**：刷新页面或切换会话不会中断任务，支持排队、打断、继续、审批处理和全局结果追踪。
- 📁 **项目与文件工作区**：按项目组织会话和素材，使用空白项目或已有本机目录，并支持文本编辑以及图片、PDF 和安全 HTML 预览。
- 🧩 **技能与 MCP 扩展**：浏览和安装技能，从输入框直接调用，并通过 Codex 原生配置管理 MCP 服务。
- 📅 **长期计划任务**：在长期专属会话中创建、编辑、暂停、恢复或立即执行定时任务，完整保留每次 Run 的历史。
- 🧠 **记忆与摘要**：显式管理全局、项目和线程记忆，生成版本化摘要，并保存每次 Run 可复现的输入快照。
- 🔐 **受控本地 Runtime**：SQLite 数据、附件和 Run 日志默认保存在本机，执行项目权限与审批，并在导出诊断信息前脱敏。
- 💻 **跨平台 Web 与 Desktop**：在 macOS、Windows 和 Linux 的浏览器或 Electron Desktop 中使用同一套 React 前端与通用工作流，只有真正依赖操作系统的能力保持平台专属。
