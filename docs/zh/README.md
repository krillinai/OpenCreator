...
- 🤖 **原生执行**：复用 Codex 的模型、推理、工具、会话、技能和 MCP，不重复实现 Agent 引擎。
- ⚙️ **后台运行**：刷新或切换会话不中断任务，支持排队、打断、继续和全局追踪。
- 📁 **项目文件**：按项目组织会话与素材，支持文本编辑以及图片、PDF 和安全 HTML 预览。
...
- 💻 **跨端一致**：Web 与 Desktop 共用前端和工作流，仅隔离原生系统能力。
