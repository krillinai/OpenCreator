...
export function CreatorServicesSettingsView(props: {
  ...
  async function reset() {
    ...
    const confirmed = await confirm({
      ...
      description: l(
        '这会清除已保存的 Key 和服务设置，并恢复为默认配置。',
        'This clears saved keys and service settings, then restores the defaults.'
      ),
  ...
  return (
    <>
      <SettingsFieldset
        ...
        description={l('用于翻译、脚本处理和其他文本任务，支持 OpenAI API 兼容服务。', 'Used for translation, script processing, and other text tasks through OpenAI-compatible services.')}
      >
        <OpenAiFields
          id="llm"
